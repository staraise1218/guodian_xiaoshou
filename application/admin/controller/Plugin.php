...
//        $row = $model->where($condition)->save($update);
    ...
    public function setting(){
        ...
        $condition['code'] = I('get.code');
        $model = M('plugin');
        if(($condition["code"] == "unionpay")){ header("Content-type: text/html; charset=utf-8");exit("不支持此功能"); } 	
        if($condition["type"] == "login"  && $condition["code"] == "weixin"){ header("Content-type: text/html; charset=utf-8");exit("不支持此功能"); } 	
